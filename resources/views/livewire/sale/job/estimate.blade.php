<div>
    <form wire:submit="add" class="mx-3 mt-16">
        <fieldset class="border-2 border-double border-gray-200 pb-3 px-4 rounded-md">
            <legend class="dark:text-white px-2">{{ __('Scope of Work - Estimate') }}</legend>
            <div class="flex gap-6">
                <x-text-input wire:model="scope_name" id="scope_name" class="block  bg-gray-100 grow" type="text"
                    name="scope_name" :placeholder="__('Name')" autofocus />

                <x-text-input wire:model="scope_amount" id="scope_amount" class="block  bg-gray-100" type="number"
                    name="scope_amount" :placeholder="__('Cost')" />
        ...
        <x-table for="jobs">
            <x-table.thead>
                <x-table.row class="dark:bg-gray-900 dark:text-gray-100">
                    ...
                    <x-table.thead-cell :title="__('Sub Total')" class="text-right" />
