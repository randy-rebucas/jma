            ...
            <x-table.tbody class="dark:border-gray-500">
                @forelse ($items as $item)
                    <x-table.row class=" dark:bg-gray-700 dark:text-white" wire:loading.class="opacity-50">
                        ...
                        <x-table.tbody-cell :item="$item->receiving_payment->payment_amount" :action="true" class="text-right font-semibold">
                            @currency($item->receiving_payment->payment_amount)
                        </x-table.tbody-cell>
                    </x-table.row>
                @empty
