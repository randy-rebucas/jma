<div class="mt-2 border-t-2 border-indigo-500 pb-1 shadow">
    @if ($content->count() > 0)
        <x-table for="scope">
            <x-table.thead>
                <x-table.row class="dark:bg-gray-900 dark:text-gray-100">
                    <x-table.thead-cell title="Scope of work" class="text-left w-64" />
                    <x-table.thead-cell title="Price" class="text-right w-24" />
                    <x-table.thead-cell title="Sub Total" class="text-right w-24" />
            ...
            <x-table.tbody class="dark:border-gray-500">
                @foreach ($content as $item)
                    <x-table.row class="bg-white dark:bg-gray-700 dark:text-white" wire:loading.class="opacity-50">
                        <x-table.tbody-cell :item="$item->name" class="md:py-1 w-64" />
