<x-slot name="header">
    ...
    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
        {{ __('Create, Edit, Delete and Search Expenses.') }}
    </p>
</x-slot>
                ...
                <div class="space-y-6">
                    <div class="flex justify-between">
                        <x-secondary-button class="ms-3 py-3"
                            wire:click="$dispatch('openModal', { component: 'expense.create-expense' })">
                            {{ __('Create Expense') }}
                        </x-secondary-button>
                    </div>
                            ...
                            <x-table.tbody class="dark:border-gray-500">
                                @forelse ($expenses as $expense)
                                    <x-table.row class="bg-white dark:bg-gray-700 dark:text-white"
                                        ...
                                        <x-table.tbody-cell :item="$expense->amount" :action="true" class="text-right">
                                            @currency($expense->amount)
                                        </x-table.tbody-cell>
                                        <x-table.tbody-cell :item="$expense->id" class="text-right" :action="true">
                                            <button type="button" class="btn btn-info m-1 font-medium underline"
                                                wire:click="$dispatch('openModal', {component: 'expense.edit-expense', arguments: {expense: {{ $expense }} }})">
                                ...
                                @empty
                                    <x-table.row class="bg-white dark:bg-gray-700 dark:text-white">
                                        <x-table.tbody-cell colspan="6" :item="__('No expense found!!')" />
                                    </x-table.row>
                                @endforelse
                            </x-table.tbody>
                        </x-table>
